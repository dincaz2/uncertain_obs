...
from software.sfl_diagnoser.Diagnoser import ExperimentInstance, diagnoserUtils
from software.sfl_diagnoser.Planner.mcts import mcts
...
def main_mcts(ei, approach, iterations=DEFAULT_BUDGET):
    ...
    while (not state.isTerminal()) and ( not state.AllTestsReached()):
        ...
        state = ExperimentInstance.addTests(state, action)
