import software.sfl_diagnoser.Diagnoser
...
class dynamicSpectrum(object):
    ...
    def convertToFullMatrix(self):
        ...
        ans.matrix = list(map(lambda test: list(map(lambda comp: 1 if comp in test else 0, range(len(self.probabilities)))), self.TestsComponents))
        return ans
    ...
    def get_tests_by_error(self, error):
        return list(map(lambda test: test[1], filter(lambda test: self.error[test[0]] == error, enumerate(self.TestsComponents))))
    ...
    def optimize(self):
        ...
        new_names = []
        new_tests = []
        new_probabilities = list(map(lambda p: p[1], filter(lambda p: p[0] in failed_components, enumerate(self.probabilities))))
        new_error = []
        for name, test, e in zip(self.tests_names, self.TestsComponents, self.error):
