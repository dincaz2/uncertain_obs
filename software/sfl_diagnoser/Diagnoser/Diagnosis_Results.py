...
from software.sfl_diagnoser.Diagnoser.Ochiai_Rank import Ochiai_Rank
...
class Diagnosis_Results(object):
    ...
    def get_initial_tests_traces(self):
        return list(map(lambda test: (sorted(test[1]), self.error[test[0]]),
    ...
    def _get_tests_by_error(self, error):
        ...
        return dict(list(filter(lambda test: self.error[test[0]] == error, tests)))
