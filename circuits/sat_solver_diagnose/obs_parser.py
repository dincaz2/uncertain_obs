...
def parse_file(obs_path, index_in_file):
    ...
    with open(obs_path) as f:
        file_content = f.read()
    system_name, inputs, outputs = parse(file_content, index_in_file)
    ...
    return system_name, inputs, outputs, create_all_combinations(len(outputs))
...
def parse(file_content, index_in_file):
    ...
    return system_name, inputs, outputs
...
def create_output_probability(output_list):
    """
    This function randomizes a probability for each output
    """
    return [(output, round(random.uniform(0.5, 1), __digits_after_the_dot)) for output in output_list]


def create_all_combinations(size):
    """
    This function creates all 2^size combinations of outputs
    """
    # This format function transforms an integer to its binary representatin with a padding of %size zeroes
    return [list(format(i, f'0{size}b')) for i in range(1 << size)]