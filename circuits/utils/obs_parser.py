...
def parse_all_obs(obs_path):
    with open(obs_path) as f:
        file_content = f.read()
    return [parse(obs_string) for obs_string in file_content.split(__observations_delimiter) if obs_string]
...
def parse_file(obs_path, index_in_file):
    ...
    try:
        ...
        return None
    inputs, outputs = parse(obs_string)
